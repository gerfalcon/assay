...
`ratchet import` reads DCM's JSON directly; there is no converter to own. Say
`--report-all` out loud, or the metrics are only the breaches, and
`--no-fatal-found`, or DCM fails the build before ratchet gets a say.
